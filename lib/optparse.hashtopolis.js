...
let switches = [
	...
	['--outfile-format CSV', "Outfile format to use, separated with commas"],
	...
	['--cpu-affinity CSV', "Locks to CPU devices, separated with commas"],
	...
	['-d', '--backend-devices CSV', "Backend devices to use, separated with commas"],
	['-D', '--opencl-device-types CSV', "OpenCL device-types to use, separated with commas"],
	...
	['-j', '--rule-left MESSAGE', "Single rule applied to each word from left wordlist"], // TODO: Check Rule format
	['-k', '--rule-right MESSAGE', "Single rule applied to each word from right wordlist"], // TODO: Check Rule format
	...
	['--generate-rules-func-max NUMBER', "Force max X functions per rule"],
	...
	['--brain-client-features NUMBER', "Define brain client features, see below"],
	['--brain-host MESSAGE', "Brain server host (IP or domain)"], // TODO: Check IP or Domain format
	['--brain-port NUMBER', "Brain server port"], // TODO: Check Port Value (1 - 65536)
...
parser.filter('single_char', function(value) {
	if(value.length != 1) {
		throw "Single character filter mismatch";
	}
	return value;
});
parser.filter('encoding', function(value) {
	// TODO: Complete Encoding List: http://www.iana.org/assignments/character-sets
	var encodings = ['us-ascii', 'utf-8', 'utf-16', 'utf-32le', 'iso-8859-15']; // Incomplete, to be extended
	if(encodings.indexOf(value) == -1) {
		throw "Invalid encoding standards";
	}
	return value; 
});
parser.filter('csv', function(value) {
	var tokens = value.split(',');
	for(var i = 0; i < tokens.length; i++) {
		if(parseInt(tokens[i]) == NaN) {
			throw "Invalid comma-separated value: " + tokens[i];
		}
	}
	return value;
});
parser.filter('csv_hex', function(value) {
	var tokens = value.split(',');
	for(var i = 0; i < tokens.length; i++) {
		if(parseInt(tokens[i], 16) == NaN) {
			throw "Invalid comma-separated hex value: " + tokens[i];
		}
	}
	return value;
});
parser.filter('percent', function(value) {
	if(parseInt(value) == NaN) {
		throw "Invalid percent value: " + value;
...
function validateHashcatCommand(opt) {
	if(opt.attackType == 0) { // 0: Word List Attack
		// Required Dictionary
		...
		return {"result": false, "reason": "Missing / Unsupported Attack Type (Supported Types: 0, 3)"};
	}
}